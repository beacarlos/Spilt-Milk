...
class FavoritesViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if cellData[indexPath.row].tipo  == 0 {
            ...
            postCell.commentButton.addTarget(self, action: #selector(segueComment), for: .touchUpInside)
            ...
            postCell.commentLarge.addTarget(self, action: #selector(segueComment), for: .touchUpInside)
            
            return postCell
        }
    }
    
    @objc func segueComment(){
